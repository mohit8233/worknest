...
const Header = () => {
  ...
  const navClass = ({ isActive }) =>
    isActive ? "text-blue-400 border-b-2 border-blue-400 pb-1" : "hover:text-blue-400";
    ...
    <>
      <header className="sticky top-0 z-50 border-b border-blue-900/20 bg-blue-950/95 text-white shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-950/95">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <NavLink to="/" className="flex items-center gap-2">
            ...
            <span className="text-2xl font-bold">Work<span className="text-blue-500">Nest</span></span>
          </NavLink>
          ...
          <nav className="hidden md:flex items-center gap-8 font-medium">
            ...
            {!user ? (
              ...
              <div className="flex items-center gap-4">
                <NavLink to={dashboardLink} className="flex items-center gap-2 hover:text-blue-400">
                  <FaUserCircle className="text-2xl" />
                  <span>{user.name}</span>
                </NavLink>
                <button onClick={handleLogout} className="bg-red-500 px-3 py-1 rounded-lg text-sm hover:bg-red-600">Logout</button>
              </div>
            )}

            <button onClick={toggleTheme} className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white ring-1 ring-white/20 transition hover:bg-white/20">
      ...
      {menuOpen && (
        <div className="md:hidden px-6 py-6 shadow-lg bg-blue-950 text-slate-200 dark:bg-slate-950 dark:text-white flex flex-col gap-5">
          ...
          {user && <NavLink to={dashboardLink} onClick={() => setMenuOpen(false)} className="text-white font-semibold">Dashboard</NavLink>}
          {!user ? <NavLink to="/login" onClick={() => setMenuOpen(false)} className="text-white font-semibold">Login</NavLink> : <button onClick={handleLogout} className="text-red-400 text-left font-semibold">Logout</button>}
        </div>
      )}
    </>
  );
